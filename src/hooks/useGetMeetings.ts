...
export const useGetMeetings = ({
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        setIsAllMettingsDataLoading(true);
        setAllUsersDataStatus("loading");
        setError(null);

        const data = await getAllMeetingsFn({
          pagination,
          sorting,
          columnFilters,
          leadId,
        ...
        setIsAllMettingsDataLoading(false);
      }
    };
  ...
  return {
    ...
    error,
    refetch,
    appliedFilters: allMeetingsData?.appliedFilters || {}, // Return applied filters
  };
};
